...
class Synthesizer:
    ...
    def emphasis_strong(self, input, output):
        self.change_voice(input, output, speed=0.85, volume=2.5)

    def emphasis_moderate(self, input, output):
        self.change_voice(input, output, speed=0.95, volume=1.3)

    def emphasis_reduce(self, input, output):
        self.change_voice(input, output, speed=1.5, volume=0.9)

    def change_speed_slow(self, input, output):
        self.change_voice(input, output, speed=0.6)

    def change_speed_medium(self, input, output):
        self.change_voice(input, output, speed=1.0)

    def change_speed_fast(self, input, output):
        self.change_voice(input, output, speed=1.5)
    ...
    def change_volume_medium(self, input, output):
        self.change_voice(input, output, volume=1.0)

    def change_volume_loud(self, input, output):
        self.change_voice(input, output, volume=1.9)

    def change_pitch_low(self, input, output):
        self.change_voice(input, output, pitch=13000)

    def change_pitch_medium(self, input, output):
        self.change_voice(input, output, pitch=16000)

    def change_pitch_high(self, input, output):
        self.change_voice(input, output, pitch=20000)
        ...
        for res in results:
            ...
            if "emphasis" == res[1]:
                ...
                if res[2]['level'] == 'strong':
                    wav = self.synthesize_normal(text_)
                    audio.save_wav(wav, "emphasis_strong.wav", sr=16000)
                    self.emphasis_strong('emphasis_strong.wav', 'strongout_emphasis.wav')
                    wav = audio.load_wav('strongout_emphasis.wav', 16000)
                    wav = np.concatenate((np.zeros(1000), wav))
                    ...
                    wav = self.synthesize_normal(text_)
                    audio.save_wav(wav,"emphasis_moderate.wav", sr=16000)
                    self.emphasis_moderate('emphasis_moderate.wav', 'moderateout_emphasis.wav')
                    wav = audio.load_wav('moderateout_emphasis.wav', 16000)
                    wav = np.concatenate((np.zeros(1000), wav))
                    ...
                    wav = self.synthesize_normal(text_)
                    audio.save_wav(wav,"emphasis_reduce.wav", sr=16000)
                    self.emphasis_reduce('emphasis_reduce.wav', 'reducedout_emphasis.wav')
                    wav = audio.load_wav('reducedout_emphasis.wav', 16000)
                    wav = np.concatenate((np.zeros(1000), wav))
                ...
                for key in res[2].keys():
                    if key=='rate':
                        if res[2]['rate'] == "medium":
                            wav = self.synthesize_normal(text_)
                            audio.save_wav(wav,"change_speed_medium.wav", sr=16000)
                            self.change_speed_medium('change_speed_medium.wav', 'speed_medium.wav')
                            wav = audio.load_wav('speed_medium.wav', 16000)
                            wav = np.concatenate((np.zeros(1000), wav))
            ...
            if "break" == res[1]:
                for key in res[2].keys():
                    ...
                    if key=='time':
                        if res[2]['time'][-2:] =='ms':
                            time_to_break = int(res[2]['time'][:-2])
                            wav = np.zeros(time_to_break)

                        elif res[2]['time'][-1:] == 's':
                            time_to_break = int(res[2]['time'][:-1])*1000
                            wav = np.zeros(time_to_break)

            combined_wav = np.concatenate((combined_wav, wav))
        out = io.BytesIO()
        audio.save_wav(combined_wav, out, sr=16000)
        return out.getvalue()
