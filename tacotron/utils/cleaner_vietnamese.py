...
import re
from .rules import _spec_char, _currency, _d_unit, _w_unit, _number, short_dict, name_dict, name_brand_dict, countries_dict, location_dict, capital_dict, voca_dict, _char_
    ...
    try:
        ...
        if num // 10 > 0:
            if num >= 20:
                if num % 10 != 0:
                    if num % 10 == 1:
                        return num_to_text(num // 10, 0) + u" mốt"
                    if num % 10 == 5:
                        return num_to_text(num // 10, 0) + u" lăm"
                    return num_to_text(num // 10, 0) + u"  " + num_to_text(num % 10, 0)
                else:
                    return num_to_text(num // 10, 0) + u" "
            else:
                if num == 15:
                    return u"mười lăm"
                ...
                if splitChar in ['/']:
                    if len(res) == 3:
                        if int(res[0]) <= 31 and int(res[1]) <= 12 and len(res[2]) == 4:
                            return u"ngày " + num_to_text(res[0], 0) + u" tháng " + num_to_text(res[1],
                                                                                                0) + u" năm " + num_to_text(
                                res[2], 0)
                    elif len(res) == 2:
    ...
    try:
        if len(text) == 1:
            ...
            for char in text.split():
                ...
                char = w_unit(char)
                result.append(char)
            return cleaner.join_str(result)
    except:
        return text
...
class cleaner(object):
    ...
    def do(self):
        text = cleaner.collapse_whitespace(self.str)
        text = cleaner.lower(text)
        ...
        for chars in ws:
            ...
            for char in chars:
                ...
                char = _name_dict(char)
                char = _name_brand_dict(char)
                char = _capital_dict(char)
                char = _countries_dict(char)
                char = _location_dict(char)
                char = _voca_dict(char)
                char = _normalize_numbers(char)
                char = _specChar(char)
                char = c_unit(char)
                char = d_unit(char)
        ...
        return cleaner.join_str(flatten(self.result))
...
def date(text):
    ...
    if len(res) == 3:
        if int(res[0]) <= 31 and int(res[1]) <= 12 and len(res[2]) == 4:
            return u"ngày " + num_to_text(res[0], 0) + u" tháng " + num_to_text(res[1], 0) + u" năm " + num_to_text(
                res[2], 0)
    elif len(res) == 2:
        if int(res[0]) <= 31 and int(res[1]) <= 12:
            return u"ngày " + num_to_text(res[0], 0) + u" tháng " + num_to_text(res[1], 0)
...
def time(text):
    res = text.split(':')
    if len(res) == 3:
        if int(res[0]) <= 24 and int(res[1]) <= 59 and int(res[2]) <= 59:
            return u"" + num_to_text(res[0], 0) + u" giờ " + num_to_text(res[1], 0) + u" phút " + num_to_text(
                res[2], 0) + u" giây"
    elif len(res) == 2:
        if int(res[0]) <= 24 and int(res[1]) <= 59:
            return u"" + num_to_text(res[0], 0) + u" giờ " + num_to_text(res[1], 0) + u" phút"
