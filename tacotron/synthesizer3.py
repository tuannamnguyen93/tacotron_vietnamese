...
class Synthesizer:
    ...
    def synthesis_ssml(self, text_ssml):
        if "speak" in text_ssml:
            text_ssml = text_ssml.replace("speak", '')
            if "prosody:rate" in text_ssml:
                if "prosody:rate" == "x-slow" == modify_tag("x-slow"):
                    text_ssml = text_ssml.replace("prosody:rate=x-slow", '')
                    wav = self.synthesize([text_ssml], None, None, None, None)
    ...
    def synthesize_helper(self, text_k):
        ...
        for idx, text_ in enumerate(texts):
            ...
            if lentokens > 0 and lentokens < 30:
                if "prosody" in text_:
                    wav = self.synthesis_ssml(text_ssml=text_)
